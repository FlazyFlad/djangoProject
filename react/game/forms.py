...
class ContactForm(forms.Form):
    name = forms.CharField(label='Имя', max_length=255, widget=forms.TextInput(attrs={'class': 'required form-control', 'placeholder':"Name"}) )
    email = forms.EmailField(label='Email', widget=forms.TextInput(attrs={'class': 'required form-control', 'placeholder':"Email"}) )
    content = forms.CharField(widget=forms.Textarea(attrs={'cols': 60, 'rows':10, 'class': 'required form-control'}))
